...
if(!isset($_SESSION['user_name'])) {
    ...
    $sql = "select nation_id from nation where nation_name='$nationOption'";
    $result= mysqli_query($mysqli, $sql);
    while ($rowData= $result->fetch_array()) {
    ...
    $stmt = mysqli_prepare($mysqli, 'insert into GENRE_RANKING_BY_NATION_COMMENT(user_idx, genre_id, nation_id, content) values(?, ?, ?, ?)');
    mysqli_stmt_bind_param($stmt, 'iiis', $this_user, $genre_id, $nation_id, $content);
